...
- **No `withAnimation` from AppKit.** Panel state (`SelectionModel`) is mutated from AppKit — `FloatingPanel.performKeyEquivalent` (⌘K, ⌘/, ⌃⌘M), its `sendEvent` (Escape), the AppDelegate's menu items — and an animated transaction opened from there never reaches the screen: the state flips, the panel keeps drawing the previous frame, and the change only appears once something unrelated (⌘F, a click into another app) forces an update. Plain mutations from those same call sites always redraw. So the *view* declares the animation against the value (`PanelView`'s `.animation(.panelOverlay, value: selection.presentedOverlay)` and its siblings) and every mutation stays plain. Apple documents both forms as equals; this is which one works here. A shortcut that seems to do nothing is this bug, not a key-handling one — check whether the state changed before touching the key path.
